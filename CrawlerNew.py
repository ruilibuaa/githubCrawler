# -*- coding:utf-8 -*-
#__original author__ = 'Conghaoyuan'
#modified by lirui @ 2022/06/08
#python 3.X
...
class GHCrawler:
    ...
    def __init__(self):
        self.pageIndex = 1
        self.user_agent = 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.4951.64 Safari/537.36'
        #初始化headers
    ...
    def getPageItems(self,pageIndex,sort_options,search_content):
        ...
        #reg = '<a.*?v-align-middle">(.*?)</a>.*?<p.*?mb-1">(.*?)</p>.*?<div>.*?<a.*?Link--muted">(.*?)</a>.*?<span.*?programmingLanguage">(.*?)</span>.*?<relative-time.*?>(.*?)</relative-time>.*?</div>'
        reg = '<a.*?v-align-middle.*?>(.*?)</a>.*?<p.*?mb-1">(.*?)</p>.*?<a.*?Link--muted.*?>.*?</svg>(.*?)</a>.*?<span.*?programmingLanguage">(.*?)</span>.*?<relative-time.*?>(.*?)</relative-time>'
